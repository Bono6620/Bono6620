...
function fa_render_admin_page(): string
{
    ...
    if (!current_user_can('manage_options')) {
        echo '<div class="fa-empty-state">' . fa_icon('lock') . '<p>الصفحة دي للمدير بس.</p></div>';
        echo '</main></div>';
        return ob_get_clean();
    }
    ...
    echo '<section class="fa-tab-toolbar"><h1>' . fa_icon('sliders') . 'لوحة التحكم</h1></section>';
    ...
    echo '<div class="fa-admin-layout">';
    echo '<nav class="fa-admin-sidebar">';
    echo '<a href="' . esc_url(home_url('/family-admin/')) . '" class="' . ($section === 'users' ? 'is-active' : '') . '">' . fa_icon('users') . 'المستخدمون</a>';
    echo '<a href="' . esc_url(add_query_arg('fa_section', 'content', home_url('/family-admin/'))) . '" class="' . ($section === 'content' ? 'is-active' : '') . '">' . fa_icon('plus') . 'إضافة محتوى</a>';
    echo '</nav>';
    echo '<div class="fa-admin-content">';
...
function fa_render_admin_users_section(): string
{
    ...
    <div class="fa-stat-row">
        <div class="fa-stat-card"><?php echo fa_icon('users'); ?><span class="fa-stat-number"><?php echo count($users); ?></span><span class="fa-stat-label">إجمالي المستخدمين</span></div>
        <div class="fa-stat-card"><?php echo fa_icon('shield'); ?><span class="fa-stat-number"><?php echo (int) $adminCount; ?></span><span class="fa-stat-label">مدراء</span></div>
        <div class="fa-stat-card"><?php echo fa_icon('edit'); ?><span class="fa-stat-number"><?php echo (int) $editorCount; ?></span><span class="fa-stat-label">محررين</span></div>
        <div class="fa-stat-card"><?php echo fa_icon('lock'); ?><span class="fa-stat-number"><?php echo (int) $viewerCount; ?></span><span class="fa-stat-label">مشاهدين فقط</span></div>
    </div>

    <div class="fa-card fa-add-user-card">
        <h2><?php echo fa_icon('plus'); ?>إضافة مستخدم جديد</h2>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="fa-form fa-form-grid">
            ...
            <button type="submit" class="fa-button"><?php echo fa_icon('plus'); ?>إضافة المستخدم</button>
        </form>
    </div>

    <div class="fa-card fa-users-table-card">
        <h2><?php echo fa_icon('users'); ?>كل المستخدمين</h2>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="fa-inline-form" onsubmit="return confirm('متأكد إنك عايز تمسح <?php echo esc_js($u->user_login); ?>؟');">
                            ...
                            <button type="submit" class="fa-link-danger fa-as-link"><?php echo fa_icon('trash'); ?>حذف</button>
                        </form>
                        <?php endif; ?>
                    </td>
                </tr>
...
function fa_render_admin_content_section(): string
{
    ...
    <div class="fa-quickadd-grid">
        <details class="fa-card fa-details" open>
            <summary><?php echo fa_icon('tree'); ?>إضافة فرد للشجرة<?php echo fa_icon('chevron-down', 'fa-chevron'); ?></summary>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" class="fa-form fa-form-grid">
                ...
                <button type="submit" class="fa-button"><?php echo fa_icon('plus'); ?>إضافة الفرد</button>
            </form>
        </details>

        <details class="fa-card fa-details">
            <summary><?php echo fa_icon('clock'); ?>إضافة حدث زمني<?php echo fa_icon('chevron-down', 'fa-chevron'); ?></summary>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="fa-form fa-form-grid">
                ...
                <button type="submit" class="fa-button"><?php echo fa_icon('plus'); ?>إضافة الحدث</button>
            </form>
        </details>

        <details class="fa-card fa-details">
            <summary><?php echo fa_icon('scroll'); ?>إضافة وثيقة<?php echo fa_icon('chevron-down', 'fa-chevron'); ?></summary>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" class="fa-form fa-form-grid">
                ...
                <button type="submit" class="fa-button"><?php echo fa_icon('plus'); ?>إضافة الوثيقة</button>
            </form>
        </details>

        <details class="fa-card fa-details">
            <summary><?php echo fa_icon('folder'); ?>إضافة ملف<?php echo fa_icon('chevron-down', 'fa-chevron'); ?></summary>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data" class="fa-form fa-form-grid">
                ...
                <button type="submit" class="fa-button"><?php echo fa_icon('plus'); ?>إضافة الملف</button>
            </form>
        </details>
    </div>
    <?php
